...
CREATE UNIQUE INDEX member_default_agent_idx
  ON member_agent_bindings(member_ref)
  WHERE is_default = 1;
...
CREATE INDEX messages_conversation_created_idx
  ON messages(conversation_ref, created_at, rowid);
...
export function runDevelopmentBootstrap(
  ...
  const bootstrap = db.transaction(() => {
    db.prepare(
      "INSERT OR IGNORE INTO members (member_ref, display_name, created_at) VALUES (?, ?, ?)"
    ).run(input.memberRef, input.memberDisplayName, timestamp);
    db.prepare(
      "INSERT OR IGNORE INTO agents (agent_ref, display_name, created_at) VALUES (?, ?, ?)"
    ).run(input.agentRef, input.agentDisplayName, timestamp);
    db.prepare(
      `INSERT OR IGNORE INTO provider_profiles
       (provider_profile_ref, provider_kind, display_name, created_at)
       VALUES (?, 'fake', 'Local Fake Provider', ?)`
    ).run(input.providerProfileRef, timestamp);
    db.prepare(
      `INSERT OR IGNORE INTO member_agent_bindings
       (member_ref, agent_ref, provider_profile_ref, is_default, created_at)
       VALUES (?, ?, ?, 1, ?)`
    ).run(input.memberRef, input.agentRef, input.providerProfileRef, timestamp);
    db.prepare(
...
export class GatewayRepository {
  ...
  authenticateDevice(deviceRef: string, token: string): AuthenticatedDevice | null {
    ...
    if (!hashEqual(sha256(token), row.token_hash)) return null;
    return {
  ...
  createConversation(input: {
    ...
    return this.getConversationForAccess(
      conversationRef,
      input.memberRef,
      input.agentRef
    )!;
  ...
  persistSuccessfulExchange(input: {
    ...
    const persist = this.db.transaction(() => {
      ...
      this.db
        .prepare(
          `INSERT INTO provider_sessions
           ...
           DO UPDATE SET external_session_ref = excluded.external_session_ref,
      ...
      this.db
        .prepare("UPDATE conversations SET updated_at = ? WHERE conversation_ref = ?")
        .run(timestamp, input.conversationRef);
    });
    persist();
  }
}
