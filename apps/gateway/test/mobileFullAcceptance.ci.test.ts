...
describe.runIf(shouldRun)("Docker security-regression context verification", () => {
  it(
    "runs the complete security script with an isolated tracked documentation fixture",
    () => {
      ...
      if (result.error || result.status !== 0) {
        throw new Error(`docker-security-context:${result.status ?? "unknown"}`);
      }
    },
    12 * 60 * 1000
  );
