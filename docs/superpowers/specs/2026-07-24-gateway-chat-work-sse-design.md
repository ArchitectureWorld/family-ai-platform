...
apps/gateway/src/domainEvents.ts（仅在 SSE 查询能力确有需要时）
...
每个 Subscriber 拥有独立串行写队列，不允许一个慢客户端阻塞其他 Person 或其他连接。

规则：

- `response.write()` 返回 `false` 时等待 `drain`；
- 写操作始终在该 Subscriber 自己的 Promise 链中串行；
...
4. `reply.hijack()`；
5. 将原始响应交给 Hub 注册；
6. 监听 `request.raw.aborted`、`request.raw.close` 和 response error；
7. 任何一个信号只执行一次清理。
...
- 授权失效：关闭连接；
- 数据库读取失败：关闭受影响连接并让客户端重连；
- 单 Subscriber 写失败：只关闭该连接；
- Hub 一个 Person Pump 失败：不影响其他 Person，下一轮可以重新尝试；
...
使用内存或临时 SQLite 与可控 Sink，覆盖：

1. 多事件严格升序；
2. exclusive cursor；
3. 同 Person 多连接共享 Pump；
4. 不同 Person 隔离；
5. 新订阅者加入 Pump 期间不漏事件；
6. 相同事件不在单连接内重复排队；
7. 不同 Person Pump 可并行；
8. 背压等待 `drain`；
9. 队列超限只关闭慢连接；
10. 心跳不推进 Cursor；
11. Session 失效后关闭；
12. `close()` 清理计时器和 Subscriber。
...
3. 正确 SSE 响应头；
4. `afterSequence` 历史补发；
5. `Last-Event-ID` 补发；
6. 两种 Cursor 冲突返回 400；
7. 未知查询参数返回 400；
8. 新 Person 消息和 Assistant 消息实时到达；
9. 事件 `id` 与 `eventSequence` 相同；
10. 心跳没有 `id`；
11. 另一个 Person 的事件不会到达；
12. 客户端 abort 后 Subscriber 被清理；
13. Gateway 关闭不会留下悬挂计时器。
